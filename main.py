...
def get_reply( prompt_template, context):
    ...
    messages=[
        {
        "role": "system",
        "content": f"You are an expert linkedin post creator. You have to write post in the provided template {prompt_template}"
        },
        {
            "role": "system",
            "content": f"""Follow the following steps to create a post:
                        ...
                        2. Understand the detail provided by the user in {context}
                        3. Replace the original template's content with the desired content; it should not contain braces
                        4. Return the post created in the same format.
                    """
        },
        {
            "role": "user",
